...
public class AuthService {
    ...
    @Transactional(readOnly = true)
    public AuthDto.SettingsResponse getSettings(Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));

        AuthDto.SettingsResponse.SettingsResponseBuilder builder = AuthDto.SettingsResponse.builder()
                .id(user.getId())
                .email(user.getEmail())
                .name(user.getName())
                .phone(user.getPhone())
        ...
        if (user.getRole() == User.Role.COMPANY) {
            companyRepository.findByUserId(userId).ifPresent(company -> builder
                    .sido(company.getSido())
                    .sigungu(company.getSigungu())
    ...
    public AuthDto.SettingsResponse updateSettings(Long userId, AuthDto.UpdateSettingsRequest request) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));
